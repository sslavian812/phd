...
public class MultiarmedBanditUCB1 implements MultiarmedBandit {
    ...
    public MultiarmedBanditUCB1(Function<Integer, Double> action, int num) {
        this.action = action;
    ...
    public void iteration(int t) {
        ...
        double curMax = r[0] / n[0] + Math.sqrt(2 * Math.log(t) / n[0]);
        int curArm = 0;
        for (int i = 1; i < N; i++) {
            double val = r[i] / n[i] + Math.sqrt(2 * Math.log(t) / n[i]);
            if (val > curMax) {
                curMax = val;
                curArm = i;
            }
