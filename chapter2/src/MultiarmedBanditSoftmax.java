...
public class MultiarmedBanditSoftmax implements MultiarmedBandit {
    ...
    public MultiarmedBanditSoftmax(Function<Integer, Double> action, int num, double tau) {
        this.action = action;
    ...
    public void iteration(int t) {
        ...
        double del = 0;
        for (int i = 0; i < N; i++) {
            del += Math.exp((r[i] / n[i]) / tau);
        }
        double sum = 0.0;
        double val = rnd.nextDouble();
        int curArm = 0;
        for (int i = 1; i < N; i++) {
            sum += Math.exp((r[i] / n[i]) / tau) / del;
            if (val < sum) {
                curArm = i;
                break;
            }
